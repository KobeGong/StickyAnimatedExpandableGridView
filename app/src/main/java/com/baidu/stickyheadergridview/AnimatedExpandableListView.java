...
public class AnimatedExpandableListView extends ExpandableListView {
     ...
     */
    private static final int MIN_ANIMATION_DURATION = 250;
    private static final int MAX_ANIMATION_DURATION = 400;
    private static final int SINGELE_ITEM_ANIMATION_DUARTION = 70;
...
//    }
    ...
    public static abstract class AnimatedExpandableListAdapter extends BaseExpandableListAdapter {
        ...
        public final View getChildView(final int groupPosition, int childPosition, boolean isLastChild, View convertView, final ViewGroup parent) {
            ...
            if (info.animating) {
                ...
                if (listView.getDivider() != null) {
                    ...
                    dummyView.setDivider(null, parent.getMeasuredWidth(),
                            listView.getDividerHeight());
                }
                
                // Make measure specs to measure child views
                int parentWidth = parent.getWidth() - parent.getPaddingLeft() - parent.getPaddingRight();
                final int measureSpecW = MeasureSpec.makeMeasureSpec(parentWidth, MeasureSpec.EXACTLY);
                ...
                if (info.expanding && state != STATE_EXPANDING) {
                    ...
                    if(childrenCount*SINGELE_ITEM_ANIMATION_DUARTION>MAX_ANIMATION_DURATION){
                        duartion=MAX_ANIMATION_DURATION;
                    }else if(childrenCount*SINGELE_ITEM_ANIMATION_DUARTION<MIN_ANIMATION_DURATION){
                        duartion=MIN_ANIMATION_DURATION;
                    }else{
                        duartion=childrenCount*SINGELE_ITEM_ANIMATION_DUARTION;
                    }
                    ani.setDuration(duartion);
        ...
        public final int getChildrenCount(int groupPosition) {
            ...
            GroupInfo info = getGroupInfo(groupPosition);
            if (info.animating) {
                count= info.firstChildPosition + 1;
            } else {
                count= getRealChildrenCount(groupPosition);
            }
            return count;
        }
